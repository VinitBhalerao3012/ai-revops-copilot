...
def load_csv_to_db(uploaded_file):
    """Load uploaded CSV into in-memory SQLite database"""
    try:
        df = pd.read_csv(uploaded_file)
        table_name = uploaded_file.name.replace('.csv', '').replace('-', '_').replace(' ', '_').lower()
        
        if 'db_conn' not in st.session_state or st.session_state.get('data_mode') == 'sample':
            conn = sqlite3.connect(':memory:')
            st.session_state['db_conn'] = conn
...
def get_schema():
    ...
    for table in tables:
        try:
            ...
            schema += f"Table: {table}\nColumns: {cols}\nSample values: {df.head(1).to_dict('records')}\n\n"
        except:
            pass
...
with st.sidebar:
    ...
    if data_option == "🎯 Use Sample RevOps Data":
        ...
        if 'data_mode' in st.session_state and st.session_state['data_mode'] == 'sample':
        ...
        if uploaded_files:
            for uploaded_file in uploaded_files:
                ...
                if error:
                    ...
                    st.success(f"✅ {uploaded_file.name} loaded as table: **{table_name}**")
                    st.caption(f"{len(df)} rows × {len(df.columns)} columns")
            
            if st.session_state.get('tables'):
                st.info(f"Active tables: {', '.join(st.session_state['tables'])}")
    
    # Auto-load sample data if nothing loaded yet
    if 'db_conn' not in st.session_state:
        setup_sample_database()
...
if page == "💬 AI Copilot Chat":
    ...
    with st.expander("💡 Example questions you can ask"):
        if st.session_state.get('data_mode') == 'csv':
            ...
            st.write("Try asking:")
            st.write("→ Show me all records from " + (tables[0] if tables else 'your table'))
            st.write("→ How many rows are in each table?")
            st.write("→ Show me the top 10 rows")
            st.write("→ What are the unique values in [column name]?")
